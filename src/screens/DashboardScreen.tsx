...
const DashboardScreen: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            const [loadedAccounts, loadedTransactions, loadedGoals] = await Promise.all([
            ...
            for (let i = 5; i >= 0; i--) {
                ...
                const monthTransactions = transactions.filter((t: Transaction) => {
                    const tDate = new Date(t.date);
                    return tDate.getMonth() === normalizedMonth && tDate.getFullYear() === targetYear;
                });
            ...
            const totalExpense = Object.values(categoryMap).reduce((sum: number, val: number) => sum + val, 0);
    ...
    // Calculations for Custom Bar Chart
    const maxChartValue = useMemo(() => {
        const max = Math.max(...monthlyData.flatMap(d => [d.income, d.expense]), 100);
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.header}>
                <View>
                    ...
                    <Text style={[styles.username, { color: colors.text }]}>Prince Kumar</Text>
                </View>
            ...
            >
                ...
                <View style={[styles.balanceCard, { backgroundColor: colors.primary }]}>
                    ...
                    <View style={styles.walletIconContainer}>
                         <MaterialCommunityIcons name="wallet-outline" size={32} color="rgba(255,255,255,0.8)" />
                    </View>
                </View>
                ...
                <View style={styles.statsRow}>
                    <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
                        ...
                        <Text style={[styles.trendText, { color: incomeTrend.isPositive ? colors.success : colors.danger }]}>
                            {incomeTrend.isPositive ? '↑' : '↓'} {incomeTrend.percentage.toFixed(0)}%
                        </Text>
                    </View>
                    ...
                    <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
                        ...
                        <Text style={[styles.trendText, { color: expenseTrend.isPositive ? colors.danger : colors.success }]}>
                            {expenseTrend.isPositive ? '↑' : '↓'} {expenseTrend.percentage.toFixed(0)}%
                        </Text>
                    </View>
                </View>
                ...
                <View style={[styles.sectionContainer, { backgroundColor: colors.surface }]}>
                    <View style={styles.sectionHeader}>
                        ...
                        <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>Last 6 Months</Text>
                    </View>
                    ...
                    <View style={styles.chartContainer}>
                        ...
                        <View style={styles.gridContainer}>
                            {gridLines.map((ratio, index) => (
                                <View key={index} style={styles.gridLineContainer}>
                        ...
                        <View style={styles.barsContainer}>
                            {monthlyData.map((data, index) => {
                                const incomeH = maxChartValue > 0 ? (data.income / maxChartValue) * chartHeight : 4;
                                const expenseH = maxChartValue > 0 ? (data.expense / maxChartValue) * chartHeight : 4;
                        ...
                        <View style={styles.legendContainer}>
                             <View style={styles.legendItem}>
                                ...
                                <Text style={[styles.barAxisLabel, { color: colors.textSecondary }]}>Income</Text>
                            </View>
                             <View style={styles.legendItem}>
                                <View style={[styles.legendDot, { backgroundColor: colors.danger }]} />
                                <Text style={[styles.barAxisLabel, { color: colors.textSecondary }]}>Expense</Text>
                            </View>
                        </View>
                    </View>
                </View>
                ...
                {categoryExpenses.length > 0 && (
                    <View style={[styles.sectionContainer, { backgroundColor: colors.surface }]}>
                        <View style={styles.sectionHeader}>
                            ...
                            <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>Jan 2026</Text>
                        </View>
                                ...
                                <View key={index} style={styles.horizontalBarItem}>
                                    <View style={styles.barLabelContainer}>
                                        <View style={styles.barLabelLeft}>
                                            ...
                                            <Text style={[styles.barLabel, { color: colors.text }]}>{cat.category}</Text>
                                        </View>
                                        <Text style={[styles.barValue, { color: colors.text }]}>{formatCurrency(cat.amount)}</Text>
                                    </View>
                ...
                <View style={[styles.sectionContainer, { backgroundColor: colors.surface }]}>
                    ...
                    {topGoals.length > 0 ? (
                                ...
                                <View style={[styles.goalProgress, { backgroundColor: colors.background }]}>
                                    <View
                                        style={[
                                            ...
                                            { width: `${(goal.savedAmount / goal.targetAmount) * 100}%`, backgroundColor: colors.success },
                                        ]}
                                    />
                                </View>
                            </View>
                        ...
                        <Text style={[styles.noGoalsText, { color: colors.textSecondary }]}>No savings goals yet.</Text>
                    )}
                </View>
            </ScrollView>
...
const styles = StyleSheet.create({
    ...
    gridContainer: {
        ...
        left: 0,
        right: 0,
        bottom: spacing.md + 24, // Leave space for labels
        justifyContent: 'space-between',
    },
    gridLineContainer: {
        flexDirection: 'row',
    ...
    barsContainer: {
        ...
        alignItems: 'flex-end',
        marginLeft: 40, // Match grid label width
